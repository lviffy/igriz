  ...
  useEffect(() => {
    async function fetchProjectData() {
      ...
      try {
        ...
        const project = projects.find((p: { name: string | string[] }) => p.name.includes(`igriz-diy-${chatNumber}`));
      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <a
            ...
            className="inline-flex items-center justify-center w-8 h-8 rounded hover:bg-igriz-elements-item-backgroundActive text-igriz-elements-textSecondary hover:text-[#000000] z-50"
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
        ...
        <Tooltip.Portal>
          <Tooltip.Content
            className="px-3 py-2 rounded bg-igriz-elements-background-depth-3 text-igriz-elements-textPrimary text-xs z-50"
            sideOffset={5}
          >
            {deploymentUrl}
            <Tooltip.Arrow className="fill-igriz-elements-background-depth-3" />
          </Tooltip.Content>
        </Tooltip.Portal>
      </Tooltip.Root>
    </Tooltip.Provider>
