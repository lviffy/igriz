  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__igrizThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__igrizArtifact__', '__igrizThought__', '__igrizQuickAction', '__igrizSelectedElement__'],

      // ['className', '__igrizThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__igrizArtifact__', '__igrizThought__', '__igrizQuickAction'],
    ],
  },
  strip: [],
};
...
const limitedMarkdownPlugin: Plugin = () => {
  return (tree, file) => {
    ...
    visit(tree, (node: UnistNode, index, parent: UnistParent | undefined) => {
      if (
        index == null ||
        !parent ||
