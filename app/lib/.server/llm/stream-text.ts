...
function sanitizeText(text: string): string {
  let sanitized = text.replace(/<div class=\\"__igrizThought__\\">.*?<\/div>/s, '');
  sanitized = sanitized.replace(/<think>.*?<\/think>/s, '');
  sanitized = sanitized.replace(/<igrizAction type="file" filePath="package-lock\.json">[\s\S]*?<\/igrizAction>/g, '');
  ...
  if (!modelDetails) {
    ...
    if (!modelDetails) {
      ...
      if (provider.name === 'Google' && currentModel.includes('2.5')) {
        throw new Error(
          `Model "${currentModel}" not found. Available Gemini models include: gemini-2.5-pro, gemini-2.5-flash, gemini-1.5-pro, gemini-2.0-flash, gemini-1.5-flash. Please select a valid model.`,
        );
      }
