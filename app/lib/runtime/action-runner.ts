...
import type { igrizShell } from '~/utils/shell';
...
export type FailedActionState = igrizAction &
...
export class ActionRunner {
  ...
  async #runStartAction(actionId: string, action: ActionState) {
    ...
    if (resp?.exitCode != 0 && (await this.#shouldInstallDependenciesBeforeRetry(action.content, resp?.output))) {
      const installCommand = await this.#getDependencyInstallCommand();

      if (installCommand) {
        logger.debug(`Start failed due to missing dependencies. Running install command: ${installCommand}`);

        const installResp = await shell.executeCommand(this.runnerId.get(), installCommand, () => {
          logger.debug(`[shell]:Aborting dependency install before retry\n\n`, action);
          action.abort();
        });
        ...
        resp = await shell.executeCommand(this.runnerId.get(), action.content, () => {
          logger.debug(`[${action.type}]:Aborting Action Retry\n\n`, action);
          action.abort();
        });
      }
    }
  ...
  async #shouldInstallDependenciesBeforeRetry(command: string, output?: string): Promise<boolean> {
    const trimmedCommand = command.trim();
    const packageManagerCommand = /^(npm|pnpm|yarn|bun)\s+(run\s+)?(dev|start|preview|serve)/;

    if (!packageManagerCommand.test(trimmedCommand)) {
      return false;
    }
    ...
    const missingDependencyPatterns = [
      ...
      /MODULE_NOT_FOUND/i,
      /sh: .* not found/i,
      /jsh: command not found/i,
    ];

    const hasMissingDependencySignal = missingDependencyPatterns.some((pattern) => pattern.test(output || ''));

    if (!hasMissingDependencySignal) {
      return false;
    }
  ...
  async #getDependencyInstallCommand(): Promise<string | undefined> {
    try {
      const webcontainer = await this.#webcontainer;
      const packageJsonRaw = await webcontainer.fs.readFile('package.json', 'utf-8');
      ...
      try {
        await webcontainer.fs.readFile('bun.lock', 'utf-8');
        return 'bun install';
      } catch {}

      try {
        await webcontainer.fs.readFile('bun.lockb', 'binary');
        return 'bun install';
      } catch {}
