...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _commandPatternMap = new Map<string, RegExp>([
    ['npm', /^(npm|yarn|pnpm|bun)\s+(install|run|start|build|dev|test|init|create|add|remove)/],
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<igrizArtifact') || input.includes('</igrizArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<igrizArtifact id="${artifactId}" title="${title}" type="bundled">
<igrizAction type="file" filePath="${filePath}">
${content}
</igrizAction>
</igrizArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<igrizArtifact id="${artifactId}" title="Shell Command" type="shell">
