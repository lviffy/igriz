...
export default class GoogleProvider extends BaseProvider {
  ...
  staticModels: ModelInfo[] = [
     ...
     * Gemini 2.5 Pro/Flash: 1M context in current Google AI Studio model line
  ...
  async getDynamicModels(
    ...
    return data.map((m: any) => {
      ...
      if (m.inputTokenLimit && m.outputTokenLimit) {
        ...
        contextWindow = 1000000; // Gemini 2.5 Pro fallback
      } else if (modelName.includes('gemini-2.5-flash')) {
        contextWindow = 1000000; // Gemini 2.5 Flash fallback
